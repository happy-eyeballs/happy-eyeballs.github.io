...
class Packet:
    ...
    def get_non_http_server_port(self) -> str:
        port1 = self._layers['tcp']['tcp.srcport']
        port2 = self._layers['tcp']['tcp.dstport']
        return port2 if port1 == '80' else port1
...
class TCPConnection:
    ...
    def contains_handshake(self) -> bool:
        return self.packets[0].has_tcp_syn_flag() and not self.packets[0].has_tcp_ack_flag()
...
class TestCaseIteration:
    def __init__(self, iteration_variable: int, packets_in_iteration: list[Packet]):
        self.iteration_variable = iteration_variable

        self.connections = extract_tcp_connections(packets_in_iteration)
    ...
    def calculate_time_until_first_connection(self) -> str | None:
        if len(self.dns_packets) == 0 or len(self.connections) == 0:
            return None

        # retrieve all DNS packets sent by the client to the server
        dns_request_packets = [dns_packet for dns_packet in self.dns_packets if dns_packet.is_dns_request_packet()]
...
def extract_tcp_connections(packets: list[Packet]) -> list[TCPConnection]:
    # filter for tcp packets
    packets = [packet for packet in packets if packet.is_tcp_packet()]
    ...
    for packet in packets:
        ...
        connection_id = f'{packet.ip_family()}|{packet.get_non_http_server_port()}'
    ...
    connections = [TCPConnection(packets) for packets in packets_per_connection.values() if len(packets) > 0]
    ...
    connections = [connection for connection in connections if connection.contains_handshake()]

    return connections


def extract_test_case_iterations(captured_packets: list[Packet]) -> list[TestCaseIteration]:
    ...
    for (start_index, end_index) in zip(iteration_start_indices, iteration_end_indices):
        ...
        iterations.append(TestCaseIteration(iteration_variable, packets_in_iteration))

    return iterations


def load_packet_capture(path: str, separator_packets: bool) -> list[TestCaseIteration]:
    ...
    if separator_packets:
        return extract_test_case_iterations(captured_packets)
    else:
        return [TestCaseIteration(0, captured_packets)]
