...
export DNS_A_ZONE=$(echo $LC_DNS_A | cut -d ' ' -f 2)
export DNS_AAAA_ZONE=$(echo $LC_DNS_AAAA | cut -d ' ' -f 2)

(envsubst '$BASE_DOMAIN$DNS_BIND_ADDRESS$DNS_SERIAL$DNS_A$DNS_AAAA$DNS_A_ZONE$DNS_AAAA_ZONE' < $LC_BASE_DIR/dns.zone.template) > $LC_BASE_DIR/dns.zone
